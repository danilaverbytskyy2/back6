...
if (empty($_SERVER['PHP_AUTH_USER']) ||
    ...
    md5($_SERVER['PHP_AUTH_PW']) != md5('123')) {//password
...
function GetUsers()
{
  $db = new PDO('mysql:host=localhost;dbname=db', 'log', 'pass',//your login and pass
  [PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);

  try{
    $sth = $db->prepare('SELECT id, fio, year, email, gender, biography, checkcontract FROM Applications');
      $sth->execute();
      $k = 0;
      $values = array();
      $row = $sth->fetchAll();
...
function DeleteUser($id)
{
  ...
  try{
    ...
    $sth->execute(['id' => $id]);
    $sth = $db->prepare('DELETE FROM Applications WHERE id = :id');
    $sth->execute(['id' => $id]);
  }
  catch(PDOException $e){
    print('Error : ' . $e->getMessage());
